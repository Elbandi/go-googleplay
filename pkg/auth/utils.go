...
func getSubToken(masterToken string) (string, error) {
	...
	log.Debugf("sub token response: %v", kvs)
	return kvs["auth"], nil
}
...
func doBrowserVerification(url string) (string, error) {
	...
	allocCtx, cancel := chromedp.NewExecAllocator(context.Background(), browserOptions[:]...)
	defer cancel()
...
func getPlayStoreAuthSubToken(email string, encryptedPasswd string, gsfId string, device *device.Profile) (string, error) {
	...
	if has {
		if kvs["info"] == "WebLoginRequired" {
			...
			log.Infof("Launching chrome browser ...")
			...
			log.Debugf("got oauth code: %s", oauthCode)

			masterToken, err = getMasterTokenFromOAuthCode(email, oauthCode, gsfId, device)
			if err != nil {
				return "", err
			}
		} else {
